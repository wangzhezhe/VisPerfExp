    ...
    for rank in range(0,procs,1):
        ...
        for line in fo:
            ...
            if split_str[0]=="GANG_COMM_END":
                gang_comm_end=float(split_str[4])
                all_particles.append([recvok,gang_comm_end,rank])
    ...
    for index, p in enumerate(all_particles_sorted):
        if index>0:
            ...
            for line in fo:
                ...
                if split_str[0]=="CommEnd_0":
                    adv_end_time=float(split_str[1])
                
                #if adv_start_time>4.88138e+07 and adv_start_time<5.1e+07:
                #print("debug",adv_start_time,adv_end_time)

                # get each start and end in one iteration
                # check if add them
                if adv_start_time<send_time_from_src and adv_end_time>send_time_from_src and adv_end_time<recv_time_of_dst:
...
if __name__ == "__main__":
    ...
    for index, data in enumerate(dataname):
         ...
         print("dirname",dirname_complete,"index",index)
         figid = "(d."+str(index+1)+")"
         draw_deserilize_time(axs[3][index],procs,dirname_complete, official_name[index], figid)

    for index, data in enumerate(dataname):
         dirname_complete = dirPath+"/"+data
         print("dirname",dirname_complete,"index",index)
         figid = "(e."+str(index+1)+")"
         #if official_name[index]=="Supernova":
         draw_adjust_prev_gang_time(axs[4][index],procs,dirname_complete, official_name[index], figid)

    fig.text(0.5, 0.08, 'Time (ms)', ha='center',fontsize=labelSize)
