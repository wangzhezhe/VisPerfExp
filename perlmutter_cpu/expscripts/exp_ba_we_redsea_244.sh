...
RUNDIR=/pscratch/sd/z/zw241/zw241/VisPerfStudy/Results/VisPerfExp_ba_we_redsea_32_244_${1}
CURRDIR=$(pwd)
...
--advect-seed-box-extents=0.010000,498.9,0.010000,498.9,0.010000,48.9 \
--field-name=velocity \
...
srun -N $NUM_NODE -n $NUM_RANK ../StreamlineMPI2 $DATADIR/${DATA_NAME} velocity $STEPSIZE_REDSEA $MAXSTEPS $NUM_TEST_POINTS $NUM_SIM_POINTS_PER_DOM $NXYZ true &> ./we_adv_3.log

# only one stage
srun -N $NUM_NODE -n $NUM_RANK ../StreamlineMPI2 $DATADIR/${DATA_NAME} velocity $STEPSIZE_REDSEA $MAXSTEPS $NUM_TEST_POINTS $NUM_SIM_POINTS_PER_DOM $NXYZ false &> ./we_adv_1.log
